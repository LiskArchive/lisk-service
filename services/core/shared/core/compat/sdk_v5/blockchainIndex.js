...
const indexBlocks = async job => {
	...
	try {
		...
		if (accountsFromTransactions.length) await accountsDB.upsert(trx, accountsFromTransactions);
...
// 		if (Number
// .isNaN(genesisHeight)) throw new Error('Genesis height is not set in the blockchainStore');
