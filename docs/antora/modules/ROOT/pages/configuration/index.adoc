...
+
--
[cols="2,1,3,3m",options="header",stripes="hover"]
|===
...
| number
| Job run interval to cleanup block cache. 
By default, it is set to 0.
| 0

| `JOB_SCHEDULE_CACHE_CLEANUP`
| string
| Job run cron schedule to cleanup block cache.
...
+
--
[cols="2,1,4,2m",options="header",stripes="hover"]
|===
|Environment variable
|Datatype
|Description
...
By default, it is set to run every 5th minute.
| */5 * * * *
...
By default, it is set to run every 15th minute.
| */15 * * * *
...
By default, it starts at the genesis height in full mode and the current block height at the moment of application initialization in the quick mode.
| 1
...
By default, it is set to run every 30th minute.
| */30 * * * *
...
| number
| Job run interval to cleanup cache.
By default, it is set to `0`.
| 0

| `JOB_SCHEDULE_CACHE_PURGE`
| string
| Job run cron schedule to cleanup cache.
...
| `EXPORT_S3_REGION`
| string
| Region where Amazon S3 bucket is hosted. Optional.
| eu-central-1
