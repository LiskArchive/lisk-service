...
describe('Export API', () => {
	...
	describe('Schedule file export', () => {
		it('Schedule file export from account address with interval -> return 202 Accepted', async () => {
			...
			const response = await api.get(
				`${baseUrlV3}/exports/transactions?address=${refTransaction1.sender.address}&interval=${startDate}:${endDate}`,
				httpStatus.ACCEPTED,
			);
		...
		it('Schedule file export from account publicKey with interval -> return 202 Accepted', async () => {
			const expected = { ready: false };
			const response = await api.get(
				`${baseUrlV3}/exports/transactions?publicKey=${refTransaction2.sender.publicKey}&interval=${startDate}:${endDate}`,
				httpStatus.ACCEPTED,
			);
			expect(response).toMap(goodRequestSchemaForExport);
			expect(response.data).toBeInstanceOf(Object);
		...
		it('Schedule file export from account address -> return 202 Accepted', async () => {
			const expected = { ready: false };
			const response = await api.get(
				`${baseUrlV3}/exports/transactions?address=${refTransaction3.sender.address}`,
				httpStatus.ACCEPTED,
			);
			expect(response).toMap(goodRequestSchemaForExport);
			expect(response.data).toBeInstanceOf(Object);
		...
		it('Schedule file export from account publicKey -> return 202 Accepted', async () => {
			const expected = { ready: false };
			const response = await api.get(
				`${baseUrlV3}/exports/transactions?publicKey=${refTransaction4.sender.publicKey}`,
				httpStatus.ACCEPTED,
			);
			expect(response).toMap(goodRequestSchemaForExport);
			expect(response.data).toBeInstanceOf(Object);
	...
	describe('Download csv file -> returns 200 OK', () => {
		...
		it('scheduled from account address -> 200 OK', async () => {
			...
			const response = await api.get(`${baseUrlV3}/exports/${validFileName}`, httpStatus.OK);
	...
	describe('Invalid params/request', () => {
		it('Schedule file export -> 400 when no params', async () => {
			const response = await api.get(`${baseUrlV3}/exports/transactions`, httpStatus.BAD_REQUEST);
			expect(response).toMap(badRequestSchema);
		});

		it('Schedule file export -> 400 when invalid address', async () => {
			const response = await api.get(
				`${baseUrlV3}/exports/transactions?address=lsknww5x4dv93x3euds4w72d99ouwnqojyw5qrm`,
		...
		it('Schedule file export -> 400 when invalid publicKey', async () => {
			const response = await api.get(
				`${baseUrlV3}/exports/transactions?publicKey=d517f9d9ac10a61b57d1959b88f8b5c6e8824d27a5349ec7ece44c4a027c4`,
				httpStatus.BAD_REQUEST,
			);
			expect(response).toMap(badRequestSchema);
		});
		...
		it('Schedule file export -> 400 when address with invalid interval', async () => {
			...
			const response = await api.get(
				`${baseUrlV3}/exports/transactions?address=${refTransaction1.sender.address}&interval=${invalidInterval}`,
				httpStatus.BAD_REQUEST,
			);
			expect(response).toMap(badRequestSchema);
		});
		...
		it('Schedule file export -> 400 when publicKey with invalid interval', async () => {
			const invalidInterval = '20-10-2021:20-11-2021';
			const response = await api.get(
				`${baseUrlV3}/exports/transactions?publicKey=${refTransaction2.sender.publicKey}&interval=${invalidInterval}`,
				httpStatus.BAD_REQUEST,
			);
			expect(response).toMap(badRequestSchema);
		});
		...
		it('Schedule file export -> 400 when invalid address with interval', async () => {
			const response = await api.get(
				`${baseUrlV3}/exports/transactions?address=lsknww5x4dv93x3euds4w72d99ouwnqojyw5qrm&interval=${startDate}:${endDate}`,
				httpStatus.BAD_REQUEST,
			);
			expect(response).toMap(badRequestSchema);
		});
		...
		it('Schedule file export -> 400 when invalid publicKey with interval', async () => {
			const response = await api.get(
				`${baseUrlV3}/exports/transactions?publicKey=d517f9d9ac10a61b57d1959b88f8b5c6e8824d27a5349ec7ece44c4a027c4&interval=${startDate}:${endDate}`,
				httpStatus.BAD_REQUEST,
			);
			expect(response).toMap(badRequestSchema);
		});
		...
		it('File not exists -> 404 NOT_FOUND', async () => {
			...
			const response = await api.get(`${baseUrlV3}/exports/${invalidFile}`, httpStatus.NOT_FOUND);
