...
function getServiceFullname(svc) {
    if (svc.version != null && svc.settings.$noVersionPrefix !== true) return `${typeof (svc.version) === 'number' ? `v${svc.version}` : svc.version}.${svc.name}`;
...
module.exports = {
    ...
    methods: {
                ...
                try {
                    ...
                    if (route.cors) {
                        ...
                        if (req.method == 'OPTIONS' && req.headers['access-control-request-method']) {
                            ...
                            if (route.logging) this.logResponse(req, res);
                    ...
                    } if (route.mappingPolicy == MAPPING_POLICY_RESTRICT) {
        ...
        async aliasHandler(req, res, alias) {
            ...
            if (_.isFunction(alias.handler)) {
                ...
                if (route.logging && (this.settings.logRequest && this.settings.logRequest in this.logger)) this.logger[this.settings.logRequest](`   Call custom function in '${alias.toString()}' alias`);
                ...
                if (alias.action) return this.callAction(route, alias.action, req, res, alias.type == 'stream' ? req : req.$params);
        ...
        async callAction(route, actionName, req, res, params) {
            ...
            try {
                ...
                if (route.onAfterCall) data = await route.onAfterCall.call(this, ctx, route, req, res, data);
        ...
        sendResponse(req, res, data, action) {
            ...
            if (action && action.responseHeaders) {
                Object.keys(action.responseHeaders).forEach(key => {
                    ...
                    if (key == 'Content-Type' && !responseType) responseType = action.responseHeaders[key];
                });
            }
            ...
            if (ctx.meta.$responseHeaders) {
                Object.keys(ctx.meta.$responseHeaders).forEach(key => {
                    if (key == 'Content-Type' && !responseType) responseType = ctx.meta.$responseHeaders[key];
        ...
        sendError(req, res, err) {
            // Route error handler
            if (req.$route && _.isFunction(req.$route.onError)) return req.$route.onError.call(this, req, res, err);

            // Global error handler
            if (_.isFunction(this.settings.onError)) return this.settings.onError.call(this, req, res, err);
        ...
        checkWhitelist(route, action) {
            ...
            return route.whitelist.find(mask => {
                ...
                if (_.isRegExp(mask)) return mask.test(action);
            }) != null;
        },
        ...
        createRoute(opts) {
            ...
            if (opts.bodyParsers) {
                ...
                Object.keys(bps).forEach(key => {
                    ...
                    if (bps[key] !== false && key in bodyParser) route.middlewares.push(bodyParser[key](opts));
                });
            }
            ...
            // Middlewares
            const mw = [];
            if (this.settings.use && Array.isArray(this.settings.use) && this.settings.use.length > 0) mw.push(...this.settings.use);
            ...
            if (this.settings.rateLimit) {
                ...
                if (opts.StoreFactory) route.rateLimit.store = new opts.StoreFactory(opts.window, opts);
    ...
    created() {
        ...
        // Process routes
        this.routes = [];
        if (Array.isArray(this.settings.routes)) this.settings.routes.forEach(route => this.addRoute(route));
