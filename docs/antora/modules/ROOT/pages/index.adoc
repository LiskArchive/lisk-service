...
The xref:{url_api_http}[HTTP API] provides various endpoints that offer more exhaustive information in comparison to the HTTP API of that of a normal Lisk node.
...
The xref:{url_api_rpc}[RPC-API] reference provides an overview of all available RPC requests. Almost every HTTP endpoint exposes a corresponding RPC endpoint to query over WS/WebSocket. The only exception is the `/export/download` endpoint.
