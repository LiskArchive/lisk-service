...
describe('Multisignature Transactions API', () => {
	...
	describe('Add multisignature transactions to the pool (POST)', () => {
		...
		it('Token transfer', async () => {
			const response = await api.post(`${endpoint}`, tokenTransferTrx);
			expect(response).toMap(goodRequestSchema);
			expect(response.data).toBeInstanceOf(Array);
			expect(response.data.length).toBe(1);
			response.data.forEach(multisigTxn => {
		...
		it('Delegate registration', async () => {
			const response = await api.post(`${endpoint}`, delegateRegistrationTrx);
			expect(response).toMap(goodRequestSchema);
			expect(response.data).toBeInstanceOf(Array);
			expect(response.data.length).toBe(1);
			response.data.forEach(multisigTxn => {
		...
		it('Voting', async () => {
			const response = await api.post(`${endpoint}`, votingTrx);
			expect(response).toMap(goodRequestSchema);
			expect(response.data).toBeInstanceOf(Array);
			expect(response.data.length).toBe(1);
			response.data.forEach(multisigTxn => {
		...
		it('Unlocking', async () => {
			const response = await api.post(`${endpoint}`, unlockingTrx);
			expect(response).toMap(goodRequestSchema);
			expect(response.data).toBeInstanceOf(Array);
			expect(response.data.length).toBe(1);
			response.data.forEach(multisigTxn => {
		...
		it('Proof of Misbehavior', async () => {
			const response = await api.post(`${endpoint}`, pomTrx);
			expect(response).toMap(goodRequestSchema);
			expect(response.data).toBeInstanceOf(Array);
			expect(response.data.length).toBe(1);
			response.data.forEach(multisigTxn => {
