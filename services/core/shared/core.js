...
const isProperObject = ObjectUtilService.isProperObject;
...
const getAddressByAny = async (param) => {
	...
	if (!hasPrefix(param)) {
		...
		else if (validatePublicKey(param)) return getAddressByPublicKey(param);
...
const getPublicKeyByUsername = async (username) => {
	...
	if (!Array.isArray(account.data) || isEmptyArray(account.data)) return '';
	const publicKey = account.data[0].publicKey;
	return publicKey;
};
...
const getPublicKeyByAny = async (param) => {
	...
	else if (validateAddress(param)) return getPublicKeyByAddress(param);
...
const getForgingStats = async (address) => {
	...
	try {
		...
		return Object.assign({}, result.data, result.meta);
	} catch (e) {
		return {};
	}
};
...
const getEpochUnixTime = async () => {
	...
	const epoch = nodeConstants.data.epoch;
