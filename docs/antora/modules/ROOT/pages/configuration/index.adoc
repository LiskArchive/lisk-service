...
:url_molecular_load_balancer_faq: https://moleculer.services/docs/0.14/faq.html#Why-am-I-getting-502-Bad-Gateway-when-api-gateway-is-behind-ALB-on-AWS
:source-language: js
...
| number
| Defines the number of microseconds gateway will wait before closing an idle connection.
...
| number
| Defines the maximum number of microseconds for gateway to send HTTP response headers after the client's request.
To enable, ensure `ENABLE_REVERSE_PROXY_TIMEOUT_SETTINGS` is set to true or `HTTP_RATE_LIMIT_NUM_KNOWN_PROXIES` is set to greater than 0.
