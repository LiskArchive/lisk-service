...
function compose(...mws) {
    ...
    return (req, res, done) => {
        const next = (i, err) => {
            ...
            } else if (mws[i].length < 4) mws[i].call(self, req, res, err => next(i + 1, err));
            else next(i + 1);
        };
    ...
    return new Promise((resolve, reject) => {
        compose.call(this, ...mws)(req, res, err => {
            if (err) {
                ...
                /* istanbul ignore next */
                if (err instanceof Error) return reject(new MoleculerError(err.message, err.code || err.status, err.type)); // TODO err.stack
