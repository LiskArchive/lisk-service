...
const calcTargetPairPrices = (rawPricesBySource, targetPairs = TARGET_PAIRS) => {
    const finalPrices = {};
...
const updatePricesCache = (prices) => BluebirdPromise.all(
    TARGET_PAIRS.map(
        pair => pricesCache.set(pair, JSON.stringify(prices[pair])),
    ),
);

const updatePrices = async () => {
    const rawPricesBySource = await getRawPricesBySource();
    // console.log(require('util').inspect(rawPricesBySource, false, 5, true));

    const targetPairPrices = calcTargetPairPrices(rawPricesBySource);
    console.log(require('util').inspect(targetPairPrices, false, 5, true));
